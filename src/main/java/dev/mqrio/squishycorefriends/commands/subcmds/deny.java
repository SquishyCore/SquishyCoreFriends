...
public class deny {
    public deny(CommandSender sender, String[] args) throws SQLException {
        Configuration config = new Configuration();
        String pluginPrefix = config.GetConfig().getString("prefix");

        Player player = (Player) sender;

        if(!player.hasPermission(config.GetConfig().getString("permissions.requestDeny"))) {
        ...
        if( SquishyCoreFriends.requestsCache.hasKey(requesterPlayer.uuid + ":" + requesteePlayer.uuid) ) {
            ...
            if (requesterBukkitPlayer.isOnline()) {
                requesterBukkitPlayer.getPlayer().sendMessage(ChatColor.translateAlternateColorCodes('&', pluginPrefix + config.GetConfig().getString("locale.deniedRequestRequesterVersion").replaceAll(Pattern.quote("{requestee}"), requesteePlayer.username)));
            }
            player.sendMessage(ChatColor.translateAlternateColorCodes('&', pluginPrefix + config.GetConfig().getString("locale.deniedRequestRequesteeVersion").replaceAll(Pattern.quote("{sender}"), requesterPlayer.username)));

            SquishyCoreFriends.requestsCache.remove(requesterPlayer.uuid + ":" + requesteePlayer.uuid);
        } else {
            player.sendMessage(ChatColor.translateAlternateColorCodes('&', pluginPrefix + config.GetConfig().getString("locale.nothingToDeny")));
        }
    }
}

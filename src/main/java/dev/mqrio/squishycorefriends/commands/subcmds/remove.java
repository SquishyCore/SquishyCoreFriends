...
public class remove {
    public remove(CommandSender sender, String[] args) throws SQLException {
        Configuration config = new Configuration();
        String pluginPrefix = config.GetConfig().getString("prefix");

        Player player = (Player) sender;

        if(!player.hasPermission(config.GetConfig().getString("permissions.remove"))) {
        ...
        Integer friendRevokeResult = new Actions().EndFriendship(player.getUniqueId().toString(), friendPlayer.uuid);

        if( friendRevokeResult == -1 ) {
            player.sendMessage(ChatColor.translateAlternateColorCodes('&', pluginPrefix + config.GetConfig().getString("locale.removedFriend").replaceAll(Pattern.quote("{friend}"), friendPlayer.username)));
        } else {

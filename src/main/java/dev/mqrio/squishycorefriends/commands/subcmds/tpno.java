...
public class tpno {
    public tpno(CommandSender sender, String[] args) throws SQLException {
        Configuration config = new Configuration();
        String pluginPrefix = config.GetConfig().getString("prefix");

        Player player = (Player) sender;

        if(!player.hasPermission(config.GetConfig().getString("permissions.tpRequestDeny"))) {
        ...
        if( SquishyCoreFriends.tpRequestsCache.hasKey(requesterPlayer.uuid + ":" + requesteePlayer.uuid) ) {
            ...
            if (requesterBukkitPlayer.isOnline()) {
                requesterBukkitPlayer.getPlayer().sendMessage(ChatColor.translateAlternateColorCodes('&', pluginPrefix + config.GetConfig().getString("locale.deniedTpRequestRequesterVersion").replaceAll(Pattern.quote("{requestee}"), requesteePlayer.username)));
            }
            player.sendMessage(ChatColor.translateAlternateColorCodes('&', pluginPrefix + config.GetConfig().getString("locale.deniedTpRequestRequesteeVersion").replaceAll(Pattern.quote("{sender}"), requesterPlayer.username)));

            SquishyCoreFriends.tpRequestsCache.remove(requesterPlayer.uuid + ":" + requesteePlayer.uuid);
        } else {
            player.sendMessage(ChatColor.translateAlternateColorCodes('&', pluginPrefix + config.GetConfig().getString("locale.tpNothingToDeny")));
        }
    }
}

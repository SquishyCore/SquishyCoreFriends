...
public class tpignore {
    public tpignore(CommandSender sender, String[] args) throws SQLException {
        Configuration config = new Configuration();
        String pluginPrefix = config.GetConfig().getString("prefix");

        Player player = (Player) sender;

        if(!player.hasPermission(config.GetConfig().getString("permissions.tpRequestIgnore"))) {
        ...
        if( SquishyCoreFriends.tpRequestsCache.hasKey(requesterPlayer.uuid + ":" + requesteePlayer.uuid) ) {
            player.sendMessage(ChatColor.translateAlternateColorCodes('&', pluginPrefix + config.GetConfig().getString("locale.tpIgnoredRequest").replaceAll(Pattern.quote("{sender}"), requesterPlayer.username)));

            SquishyCoreFriends.tpRequestsCache.remove(requesterPlayer.uuid + ":" + requesteePlayer.uuid);
        } else {
            player.sendMessage(ChatColor.translateAlternateColorCodes('&', pluginPrefix + config.GetConfig().getString("locale.tpNothingToIgnore")));
        }
    }
}
